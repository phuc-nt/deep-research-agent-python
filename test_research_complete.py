...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("logs/test_research_complete.log"),
            logging.StreamHandler(sys.stdout)
        ]
    )
    
    # Tạo logger
    return logging.getLogger("test-research-complete")
...
async def main():
    """
    Chạy quá trình nghiên cứu hoàn chỉnh từ đầu đến khi hoàn thành:
    ...
    try:
        ...
        response = requests.post(f"{BASE_URL}/research/complete", json=research_request)
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if status == "completed":
                    ...
                    logger.info("=== KẾT THÚC KIỂM TRA FLOW NGHIÊN CỨU HOÀN CHỈNH - THÀNH CÔNG ===")
                    return True
                    ...
                    logger.info("=== KẾT THÚC KIỂM TRA FLOW NGHIÊN CỨU HOÀN CHỈNH - THẤT BẠI ===")
                    return False
        ...
        logger.info("=== KẾT THÚC KIỂM TRA FLOW NGHIÊN CỨU HOÀN CHỈNH - TIMEOUT ===")
        return False
        
    except requests.RequestException as e:
        logger.error(f"Lỗi khi tạo yêu cầu nghiên cứu: {str(e)}")
        print(f"Lỗi: {str(e)}")
        logger.info("=== KẾT THÚC KIỂM TRA FLOW NGHIÊN CỨU HOÀN CHỈNH - LỖI ===")
        return False
